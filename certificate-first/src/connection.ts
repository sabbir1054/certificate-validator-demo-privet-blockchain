...
const chaincodeName = process.env.CHAINCODE_NAME || 'CertificateContract';
const mspId = process.env.MSP_ID || 'Org1MSP';

// Local Fabric crypto materials path
const FABRIC_CRYPTO = path.resolve(__dirname, '../../fabric-samples/test-network/organizations/peerOrganizations/org1.example.com/users/User1@org1.example.com/msp');
const keyPath = path.join(FABRIC_CRYPTO, "keystore", "priv_sk");  // Ensure the correct key file
const certPath = path.join(FABRIC_CRYPTO, "signcerts", "User1@org1.example.com-cert.pem");
...
export function getContract(): Contract {
    if (!contractInstance) throw new Error("Fabric contract not initialized yet.");
    return contractInstance;
}
